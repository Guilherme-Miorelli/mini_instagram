...
const curtir = async (req, res) => {
    ...
    try {
        ...
        if (jaCurtiu) {
            return res.status(400).json('O usuário já curtiu esta publicação');
        }
        ...
        if (!curtida) {
            return res.status(400).json('Ocorreu um erro ao curtir a publicação');
        }

        return res.status(200).json('A publicação foi curtida');
    } catch (error) {
        return res.status(400).json(error.message);
    }
}
...
const comentar = async (req, res) => {
    ...
    if (!texto) {
        return res.status(404).json('Deve-se informar um texto para comentar.');
    }
    ...
    try {
        ...
        if (!comentario) {
            return res.status(400).json('Ocorreu um erro ao comentar na publicação');
        }
...
const feed = async (req, res) => {
    ...
    //aqui a paginação virá preenchida caso seja informada na url, caso contrário será setado como 0
    const paginacao = offset ? offset : 0;

    try {
        // const postagens = knex('postagens').limit(10).offset(paginacao);
        //where está pegando postagens diferentes do id do usuario logado e irá mostrar dos outros usuarios
        ...
        for (const postagem of postagens) {
            ...
            //curtido por mim
            postagem.curtidosPorMim = curtidas.find(curtida => curtida.usuario_id === id) ? true : false;
